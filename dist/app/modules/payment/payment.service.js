...
const stripePayment = (orderId, paymentMethodId, currency) => __awaiter(void 0, void 0, void 0, function* () {
    const order = yield order_model_1.default.findById(orderId);
    if (!order)
        throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'Order not found');
    if (order.isPaid === 'yes')
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, 'You have already paid');
    ...
    if (!paymentMethod || paymentMethod.type !== 'card') {
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, 'Invalid payment method');
    }
    ...
    if (paymentIntent.status === 'succeeded') {
        ...
        if (paymentMethod.card) {
            ...
            throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, 'Card information missing');
        }
...
const paypalPayment = (orderId, email, currency) => __awaiter(void 0, void 0, void 0, function* () {
    const order = yield order_model_1.default.findById(orderId);
    if (!order) {
        throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'Order not found');
    }
    ...
    const createdPayment = yield new Promise((resolve, reject) => {
        paypal_rest_sdk_1.default.payment.create(paymentData, (error, payment) => {
            if (error) {
                reject(error);
            }
            else {
    ...
    const approvalUrl = createdPayment.links.find((link) => link.rel === 'approval_url');
    if (!approvalUrl) {
        throw new ApiError_1.default(http_status_1.default.BAD_REQUEST, 'Failed to find PayPal approval URL');
    }
...
const handlePayPalWebhookForVerifyPayment = (paymentId) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    // Retrieve the payment details from PayPal
    const payment = yield new Promise((resolve, reject) => {
        paypal_rest_sdk_1.default.payment.get(paymentId, (error, retrievedPayment) => {
            if (error) {
                reject(error);
            }
            else {
    ...
    let paymentStatus = '';
    let isPaid = false;
    if (((_a = payment.payer) === null || _a === void 0 ? void 0 : _a.status) === 'VERIFIED') {
    ...
    if (isPaid) {
        ...
        const order = yield order_model_1.default.findOneAndUpdate({ paymentId: paymentId }, { isPaid, paidAt: new Date() }, { new: true });
        if (!order)
            throw new ApiError_1.default(http_status_1.default.NOT_FOUND, 'Order not found');
        return order;
    }
